# Lasso Ridge Regression for company startuos data set

import pandas as pd
import numpy as np

# loading the data
df = pd.read_csv("E:/Data Science 18012022/Lasso Ridge Regression/50_Startups (1).csv")
...
# Generating Lasso Regression model
...
lasso_reg.fit(df.iloc[:, 1:], df.Profit)
...
# Adjusted r-square#
lasso_reg.score(df.iloc[:, 1:], df.Profit)

# RMSE
np.sqrt(np.mean((lasso_pred - df.Profit)**2))
...
ridge_reg.fit(df.iloc[:, 1:], df.Profit)
...
# Adjusted r-square#
ridge_reg.score(df.iloc[:, 1:], df.Profit)

# RMSE
np.sqrt(np.mean((ridge_pred - df.Profit)**2))
...
enet_reg.fit(df.iloc[:, 1:], df.Profit)
...
# Adjusted r-square
enet_reg.score(df.iloc[:, 1:], df.Profit)

# RMSE
np.sqrt(np.mean((enet_pred - df.Profit)**2))
